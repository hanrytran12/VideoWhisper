...
**Prerequisites:** Python 3.11 venv at `.venv`; FFmpeg + ffprobe on PATH; Ollama running (`ollama serve`) with the VLM model pulled (`ollama pull minicpm-v`); CUDA 12.1 optional (falls back to CPU). Full setup → `.Codex/docs/setup.md`.
...
| question | doc |
|----------|-----|
| How does orchestration / context / caching / the VLM two-layer cache work? | `.Codex/docs/pipeline.md` |
| What does each stage read & write? What's the JSON/output schema? | `.Codex/docs/stage_contracts.md` |
| What does each `config.yaml` field do? How do I tune for VRAM? | `.Codex/docs/config.md` |
| How do I install it? Why these pinned versions? Build errors? | `.Codex/docs/setup.md` |
